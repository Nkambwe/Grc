...
namespace Grc.Middleware.Api.Services.Compliance.Support {
    public class RegulatoryCategoryService : BaseService, IRegulatoryCategoryService
    {
        ...
        public virtual async Task<PagedResult<ObligaionResponse>> GetObligationsAsync(int pageIndex, int pageSize, bool isDeleted) {
            using var uow = UowFactory.Create();
            Logger.LogActivity($"Retrieve paged Regulatory obligations", "INFO");
            ...
            try {
                ...
                var categories = await uow.RegulatoryCategoryRepository.PageLookupAsync(pageIndex, pageSize, false,
                    category => new ObligaionResponse {
                        ...
                        Assurance = 0,
                        Issues = 0,
                        Laws = category.Regulations.Select(law => new ObligationLawResponse() {
                            Id = law.Id,
                            LawName = law.RegulatoryName,
                            Coverage = 0,
                            IsCovered = false,
                            Assurance = 0,
                            Issues = 0,
                ...
                foreach (var category in data) {
                    foreach (var law in category.Laws) {
                        if (law.Sections != null && law.Sections.Any()) {
                            //..calculate average coverage from sections
                            law.Coverage = Math.Round(law.Sections.Average(s => s.Coverage), 2);

                            //..calculate average assurance from sections
                            law.Assurance = Math.Round(law.Sections.Average(s => s.Assurance), 2);

                            //..cum issues from sections
                            law.Issues = law.Sections.Sum(s => s.Issues);

                            //..determine if law is covered (100% coverage)
                            law.IsCovered = law.Coverage >= 100;
                        }
                    }
                }
                ...
                Logger.LogActivity($"Failed to retrieve Regulatory Obligations: {ex.Message}", "ERROR");
                _ = await uow.SystemErrorRespository.InsertAsync(HandleError(uow, ex));
                throw;
            }
        }
