...
namespace Grc.Middleware.Api.Data.Entities.Configurations {
    public class MailSettingEntityConfiguration {
        public static void Configure(EntityTypeBuilder<MailSettings> builder) {
            ...
            builder.Property(s => s.MailSender).HasColumnName("mail_sender").HasColumnType("NVARCHAR(200)").IsRequired();
            builder.Property(s => s.CopyTo).HasColumnName("cc_mails").HasColumnType("NVARCHAR(MAX)").IsRequired();
            builder.Property(s => s.SystemPassword).HasColumnName("pwd_hash").HasColumnType("NVARCHAR(MAX)").IsRequired();
            builder.Property(s => s.NetworkPort).HasColumnName("nwt_port").IsRequired();
