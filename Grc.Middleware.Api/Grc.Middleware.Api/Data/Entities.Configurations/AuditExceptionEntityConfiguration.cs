...
namespace Grc.Middleware.Api.Data.Entities.Configurations
{
    public class AuditExceptionEntityConfiguration {
        public static void Configure(EntityTypeBuilder<AuditException> builder) {
            ...
            builder.Property(x => x.AuditTaskId).HasColumnName("task_id").IsRequired(false); 
            ...
            builder.HasOne(x => x.AuditTask).WithMany(t => t.AuditExceptions).HasForeignKey(s => s.AuditTaskId).IsRequired(false);
            builder.HasOne(x => x.Responsability).WithMany(t => t.Findings).HasForeignKey(s => s.ResponsabilityId);
        }
    }
}
