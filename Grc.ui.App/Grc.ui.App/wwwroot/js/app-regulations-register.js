...
let registerTable;
let registerNextId = 1000;

//..table initialization function
function initRegisterTable() {
    registerTable = new Tabulator("#registers-table", {
...
//..view/edit Record
function viewRegulatoryRecord(id) {
    let record = findRegulatoryRecord(sampleRegisterData, id);
    if (record) {
        openRegisterPanel('Edit Record', record, true);
    }
}

//..add regiter
function addRegulatoryRootRecord() {
    openRegisterPanel('Add New Record', {
        id: ++registerNextId,
        code: '',
        title: '',
        category: 'General',
...
//..add Child Record
function addRegisterChildRecord(parentId) {
    let parent = findRegulatoryRecord(sampleRegisterData, parentId);
    openRegisterPanel('Add Child Record', {
        id: ++registerNextId,
        parentId: parentId,
        code: '',
        title: '',
...
//..open slide panel
function openRegisterPanel(title, record, isEdit) {
...
function saveRegulationRecord() {
    ...
    //..save via controller
    saveRegisterViaController(recordData, isEdit, function (success) {
        if (success) {
            if (isEdit) {
                updateRegulatoryRecordInData(sampleRegisterData, recordData);
            } else {
                addRegulatoryRecordToFromData(sampleRegisterData, recordData);
            }

            table.replaceData(sampleRegisterData);
            closePanel();
...
//..save register
function saveRegulatoryRegister(data, isEdit, callback) {
...
//..delete Record
function deleteRegulatoryRecord(id) {
    if (confirm('Are you sure you want to delete this record and all its children?')) {
        ...
        */
        ...
        removeRegulatoryRecordFromData(sampleRegisterData, id);
        registerTable.replaceData(sampleRegisterData);
        alert('Record deleted successfully!');
    }
}

// Helper Functions
function findRegulatoryRecord(data, id) {
    for (let item of data) {
        if (item.id === id) return item;
        if (item._children) {
            let found = findRegulatoryRecord(item._children, id);
...
function addRegulatoryRecordToFromData(data, newRecord) {
    if (newRecord.parentId) {
        let parent = findRegulatoryRecord(data, newRecord.parentId);
        if (parent) {
...
function updateRegulatoryRecordInData(data, updatedRecord) {
    for (let i = 0; i < data.length; i++) {
        ...
        if (data[i]._children) {
            if (updateRegulatoryRecordInData(data[i]._children, updatedRecord)) {
                return true;
            }
        }
    }
...
function removeRegulatoryRecordFromData(data, id) {
    for (let i = 0; i < data.length; i++) {
        if (data[i].id === id) {
            data.splice(i, 1);
            return true;
        ...
        if (data[i]._children) {
            if (removeRegulatoryRecordFromData(data[i]._children, id)) {
                return true;
            }
        }
    }
