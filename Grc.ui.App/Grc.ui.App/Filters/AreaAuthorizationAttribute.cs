...
namespace Grc.ui.App.Filters {
    ...
    public class AreaAuthorizationAttribute : Attribute, IAuthorizationFilter {
        ...
        public void OnAuthorization(AuthorizationFilterContext context) {
            ...
            // Get user's role group from claims
            ...
            //..check if user's role group is allowed
            if (_allowedRoleGroups.Length > 0 && !_allowedRoleGroups.Any(rg => rg.Equals(userRoleGroup, StringComparison.OrdinalIgnoreCase))) {
                // Log unauthorized access attempt
                var logger = context.HttpContext.RequestServices.GetService<ILogger<AreaAuthorizationAttribute>>();
                logger.LogWarning($"Unauthorized area access attempt by user {user.Identity.Name} to area requiring {string.Join(", ", _allowedRoleGroups)}");
